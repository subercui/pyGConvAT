...
def get_data(raw, locs):
    ...
    # data = minus_av(data)
    data = raw.set_eeg_reference(ref_channels=['A1','A2']).get_data().clip(-0.0001, 0.0001)[None, :, :]
    ch_names = raw.info["ch_names"]
    ...
    for loc in locs:
        if loc > start_bound + 0.6*sfreq and loc < end_bound - 0.6*sfreq:
            for start_point in np.linspace(int(loc-0.45*sfreq), int(loc-0.15*sfreq), num = num_pos):
                start_point = int(start_point)
                end_point = start_point + int(win_size*sfreq)

                x_pos.append(normalization(data[:, :, start_point : end_point, :]))  # TODO: why make this 4d instead of 2d?

    rej_rate = 0.95
    for i in range(data.shape[2] - int(win_size*sfreq)):
        ACCEPT = np.random.random() > rej_rate
        for loc in locs:
            if (loc > (i - loc_dis*sfreq)) and (loc < (i + (win_size+loc_dis)*sfreq)):
                ACCEPT = False
        if ACCEPT == True:
            start_point = i
            end_point = i + int(win_size*sfreq)
            x_neg.append(normalization(data[:, :, start_point : end_point, :]))
            if len(x_neg) >= 2 * len(x_pos): break  # control the ratio of pos to neg samples
